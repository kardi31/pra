...
class News_AdminController extends MF_Controller_Action {
    ...
    protected $user;
     ...
     public function init() {
        $this->_helper->ajaxContext()
                ->addActionContext('move-category-guide', 'json')
                ->initContext();
        parent::init();
    ...
    public function listNewsDataAction() {
        ...
        $dataTables = Default_DataTables_Factory::factory(array(
            ...
            'columns' => array('x.id','xt.title', 'c.title','x.created_at','x.updated_at','x.publish_date','x.breaking_news'),
            'searchFields' => array('x.id','xt.title','c.title','x.created_at','x.updated_at','x.publish_date','x.breaking_news')
        ));
...
//                    }
    ...
    public function listCategoryGuideDataAction() {
        $i18nService = $this->_service->getService('Default_Service_I18n');
        
        $table = Doctrine_Core::getTable('News_Model_Doctrine_GuideCategory');
        $dataTables = Default_DataTables_Factory::factory(array(
            'request' => $this->getRequest(), 
            'table' => $table, 
            'class' => 'News_DataTables_GuideCategory', 
            'columns' => array('x.id','xt.title'),
            'searchFields' => array('x.id','xt.title')
        ));
        ...
        foreach($results as $result) {
            ...
            $row[] = $result->Translation[$language->getId()]->title;
                ...
                $options = '<a href="' . $this->view->adminUrl('edit-category-guide', 'news', array('id' => $result->id)) . '" title="' . $this->view->translate('Edit') . '"><span class="icon24 entypo-icon-settings"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                $options .= '<a href="' . $this->view->adminUrl('remove-category-guide', 'news', array('id' => $result->id)) . '" class="remove" title="' . $this->view->translate('Remove') . '"><span class="icon16 icon-remove"></span></a>';
...
//        $form->getElement('category_id')->addMultiOptions($categoryService->prependCategoryOptions());
    ...
    public function listCategoryGuideAction() {
        $guideService = $this->_service->getService('News_Service_Guide');
//        
        $form = $guideService->getGuideCategoryItemForm();
        $form->setAction($this->view->adminUrl('add-category-guide', 'news', array()));
        
        $categoryRoot = $guideService->getAllMainCategories();
...
//        
    ...
    public function addCategoryGuideAction() {
        $guideService = $this->_service->getService('News_Service_Guide');
//
        $form = $guideService->getGuideCategoryItemForm();
//
            ...
            if($form->isValid($this->getRequest()->getPost())) {
                try {
                    ...
                    $this->_service->get('doctrine')->getCurrentConnection()->beginTransaction();
                     ...
                     $data = $form->getValues();
                    $category = $guideService->saveCategoryGuideFromArray($data);

                    $this->_service->get('doctrine')->getCurrentConnection()->commit();
                    if($this->getRequest()->getParam('saveOnly') == '1')
                        $this->_helper->redirector->gotoUrl($this->view->adminUrl('edit-category-guide', 'news', array('id' => $category['id'] )));
                    
                    
                    $this->_helper->redirector->gotoUrl($this->view->adminUrl('list-category-guide', 'news', array()));
                } catch(Exception $e) {
                    $this->_service->get('doctrine')->getCurrentConnection()->rollback();
                    $this->_service->get('log')->log($e->getMessage(), 4);
                }
    ...
    public function editCategoryGuideAction() {
        $guideService = $this->_service->getService('News_Service_Guide');
        $i18nService = $this->_service->getService('Default_Service_I18n');
        $metatagService = $this->_service->getService('Default_Service_Metatag');
        $translator = $this->_service->get('translate');
        
        if(!$category = $guideService->getCategory($this->getRequest()->getParam('id'))) {
            throw new Zend_Controller_Action_Exception('Category not found');
        }
        
        $adminLanguage = $i18nService->getAdminLanguage();
        
        $form = $guideService->getGuideCategoryItemForm($category);
        ...
        if($this->getRequest()->isPost()) {
            if($form->isValid($this->getRequest()->getParams())) {
                try {
                    ...
                    $category = $guideService->saveCategoryGuideFromArray($values);
                    $this->view->messages()->add($translator->translate('Item has been updated'), 'success');
                    ...
                    $this->_helper->redirector->gotoUrl($this->view->adminUrl('list-category-guide', 'news'));
                } catch(Exception $e) {
                    var_dump($e->getMessage());exit;
                    $this->_service->get('doctrine')->getCurrentConnection()->rollback();
                    $this->_service->get('log')->log($e->getMessage(), 4);
    ...
    public function removeCategoryGuideAction() {
        $guideService = $this->_service->getService('News_Service_Guide');
        $metatagService = $this->_service->getService('Default_Service_Metatag');
        ...
        if($category = $guideService->getCategory($this->getRequest()->getParam('id'))) {
            try {
                ...
                $guideService->removeCategory($category);
    ...
    public function moveCategoryGuideAction() {
        $guideService = $this->_service->getService('News_Service_Guide');
        ...
        $category = $guideService->getCategory((int) $this->getRequest()->getParam('id'));
        
        $dest = $guideService->getCategory((int) $this->getRequest()->getParam('dest_id'));
        try {
            $this->_service->get('doctrine')->getCurrentConnection()->beginTransaction();
            
            $guideService->moveCategory($category, $dest, $this->getRequest()->getParam('mode', 'after'));
    ...
    public function listGuideDataAction() {
        $i18nService = $this->_service->getService('Default_Service_I18n');
        
        $table = Doctrine_Core::getTable('News_Model_Doctrine_Guide');
        $dataTables = Default_DataTables_Factory::factory(array(
            'request' => $this->getRequest(), 
            'table' => $table, 
            'class' => 'News_DataTables_Guide', 
            'columns' => array('x.id','xt.title', 'ct.title','x.created_at','x.updated_at'),
            'searchFields' => array('x.id','xt.title','ct.title','x.created_at','x.updated_at')
        ));
        ...
        foreach($results as $result) {
            ...
            $row[] = MF_Text::timeFormat($result->publish_date, 'd/m/Y H:i');
                ...
                $options = '<a href="' . $this->view->adminUrl('edit-guide', 'news', array('id' => $result->id)) . '" title="' . $this->view->translate('Edit') . '"><span class="icon24 entypo-icon-settings"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                $options .= '<a href="' . $this->view->adminUrl('remove-guide', 'news', array('id' => $result->id)) . '" class="remove" title="' . $this->view->translate('Remove') . '"><span class="icon16 icon-remove"></span></a>';
    ...
    public function addGuideAction() {
        ...
        $form = $guideService->getGuideForm();
        $form->getElement('category_id')->addMultiOptions($guideService->prependCategoryOptions());
        $metatagsForm = $metatagService->getMetatagsSubForm();
        $form->addSubForm($metatagsForm, 'metatags');
        ...
        if($this->getRequest()->isPost()) {
            if($form->isValid($this->getRequest()->getParams())) {
                try {
                    ...
                    $guide = $guideService->saveGuideFromArray($values,$this->user->getId(),$this->user->getId());
                    
                    $this->view->messages()->add($translator->translate('Item has been added'), 'success');
                    
                    $this->_service->get('doctrine')->getCurrentConnection()->commit();
                    
                    $this->_helper->redirector->gotoUrl($this->view->adminUrl('list-guide', 'news', array()));
                } catch(Exception $e) {
                    var_dump($e->getMessage());exit;
                    $this->_service->get('doctrine')->getCurrentConnection()->rollback();
                    $this->_service->get('log')->log($e->getMessage(), 4);
    ...
    public function editGuideAction() {
        ...
        if(!$guide = $guideService->getGuide($this->getRequest()->getParam('id'))) {
            throw new Zend_Controller_Action_Exception('Guide not found');
        }
        ...
        $form = $guideService->getGuideForm($guide);
        $form->getElement('category_id')->addMultiOptions($guideService->prependCategoryOptions());
        $form->getElement('category_id')->setValue($guide['category_id']);
        
        
        $metatagsForm = $metatagService->getMetatagsSubForm($guide->get('Metatags'));
        $form->addSubForm($metatagsForm, 'metatags');
        ...
        if($this->getRequest()->isPost()) {
            if($form->isValid($this->getRequest()->getParams())) {
                try {
                    ...
                    $values = $form->getValues();
                    if($metatags = $metatagService->saveMetatagsFromArray($guide->get('Metatags'), $values, array('title' => 'title', 'description' => 'content', 'keywords' => 'content'))) {
                        $values['metatag_id'] = $metatags->getId();
                    }
                    
                    $guide = $guideService->saveGuideFromArray($values,$this->user->getId());
                    ...
                    $this->_service->get('doctrine')->getCurrentConnection()->commit();
                     ...
                     if(isset($_POST['save_only'])){
                        $this->_helper->redirector->gotoUrl($this->view->adminUrl('edit-guide', 'news',array('id' => $guide->id)));
                    }

                    $this->_helper->redirector->gotoUrl($this->view->adminUrl('list-guide', 'news'));
                } catch(Exception $e) {
                    $this->_service->get('doctrine')->getCurrentConnection()->rollback();
                    $this->_service->get('log')->log($e->getMessage(), 4);
                }
    ...
    public function removeGuideAction() {
        $newsService = $this->_service->getService('News_Service_Guide');
        $metatagService = $this->_service->getService('Default_Service_Metatag');
        $metatagTranslationService = $this->_service->getService('Default_Service_MetatagTranslation');
        $photoService = $this->_service->getService('Media_Service_Photo');
        ...
        if($news = $newsService->getGuide($this->getRequest()->getParam('id'))) {
            try {
                $this->_service->get('doctrine')->getCurrentConnection()->beginTransaction();
