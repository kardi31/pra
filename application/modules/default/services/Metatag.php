...
//        Zend_Debug::dump($values['metatags']);exit;
    ...
    public function saveNewMetatagsFromArray( $values, $fallback = array()) {
...
//        $metatag->fromArray($values['metatags']);
        if(isset($values['Translation'])) {
            foreach($values['Translation'] as $language => $translation) {
                $fallbackData = isset($values['Translation'][$language]) ? $values['Translation'][$language] : $values;
                $data = $translation;
                if(!isset($data['meta_title']) || !strlen($data['meta_title'])) {
                    if(isset($fallback['title']) && strlen($fallbackData[$fallback['title']])) {
                        $data['meta_title'] = $fallbackData[$fallback['title']];
    ...
    public function setCustomViewMetatags($metatags = null, $view) {
        if(null != $metatags) {
        $view->doctype('XHTML1_RDFA');
            $metatags = $metatags[$view->language];
            
            if(strlen($metatags['title'])) {
                $view->headTitle($metatags['title'], 'SET');
                $view->headMeta($metatags['title'], 'DC.title');
                $view->headMeta($metatags['title'], 'og:title','property');
            }
            if(strlen($metatags['description'])) {
                $view->headMeta($metatags['description'], 'description');
                $view->headMeta($metatags['description'], 'DC.description');
                $view->headMeta($metatags['description'], 'og:description','property');
            }
