...
class Media_Service_Photo extends MF_Service_ServiceAbstract {
    ...
    public function copyPhoto($photo,array $options){
        $filepath = $this->photosDir . DIRECTORY_SEPARATOR . $photo['offset'] . DIRECTORY_SEPARATOR . $photo['filename'];
        $offsetDir = $this->photosDir . DIRECTORY_SEPARATOR . $photo['offset'];
        $newName = $this->createUniquefileName($photo['filename'], $offsetDir);
        copy($filepath,$this->photosDir . DIRECTORY_SEPARATOR);
        foreach($options as $dimensions => $nothing):
            copy($offsetDir . DIRECTORY_SEPARATOR . $dimensions . DIRECTORY_SEPARATOR . $photo['filename'],$offsetDir . DIRECTORY_SEPARATOR . $dimensions . DIRECTORY_SEPARATOR . $newName);
        endforeach;
        $newPhoto = $photo->copy();
